Public Class FrmDespatchReport
    ...
    Private Sub BtnDetailed_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDetailed.Click
        ...
        dtprint = ReturningDataByQ("SELECT f.Product_Name,d.Product_code,a.Desp_code,a.Desp_date,a.Desp_type,b.Location_Name FromLoc,c.Location_Name ToLoc,Status, " & _
        "d.plan_qty, d.scan_qty, e.carton_serial_no  " & _
"FROM bmr.tbl_despatch a  " & _
...
"left outer join tbl_despatchsub d on a.Desp_code=d.Desp_code  " & _
"left outer join tbl_despatchscan e on e.desp_code=a.Desp_code  " & _
"left outer join product_master f on f.Product_Code=d.product_code  " & _
   "WHERE  ('" & productCode & "'='' OR D.Product_code  ='" & productCode & "') " & _
     "AND  (CAST(A.Desp_date AS DATE)  BETWEEN  CAST('" & a & "' AS DATE) AND  CAST('" & b & "' AS DATE))")
        If dtprint.Rows.Count > 0 Then
            Dim objcrp As New CrpDespatch()
            objcrp.SetDataSource(dtprint)
            objcrp.SetParameterValue("FDate", FDate)
            objcrp.SetParameterValue("TDate", TDate)
            objcrp.SetParameterValue("RptName", " Despatch Report ")
